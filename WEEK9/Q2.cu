...
__global__ void addElement(int *A, int *B, int *C, int total) {
...
int main() {
    ...
    if (mode == 0) {
        ...
        addRow<<<blocks, threads>>>(dA, dB, dC, M, N);
    } else if (mode == 1) {
        int threads = 32;
        int blocks = (N + threads - 1) / threads;
        addCol<<<blocks, threads>>>(dA, dB, dC, M, N);
    } else {
        int total = M * N;
        int threads = 256;
        int blocks = (total + threads - 1) / threads;
        addElement<<<blocks, threads>>>(dA, dB, dC, total);
    }
